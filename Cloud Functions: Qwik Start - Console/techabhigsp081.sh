...
mkdir techabhigcp && cd techabhigcp
...
exports.GCFunction = (req, res) => {
    let message = req.query.message || req.body.message || 'Subscribe to techabhigcp';
...
DATA=$(printf 'Subscribe to techabhigcp' | base64) && gcloud functions call GCFunction --region=$REGION --data '{"data":"'$DATA'"}'
